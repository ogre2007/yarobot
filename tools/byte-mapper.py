...
def printHeatMapValue(byteToPrint, countOfByte, fileCount, byteFiller):
    ...
    if countOfByte == fileCount:
        ...
        sys.stdout.write(
            Fore.RED + "%s" % byteToPrint.encode("hex") + Fore.WHITE + "%s" % byteFiller
        )
...
def printHeatLegend(fileCount):
    ...
    if fileCount > 2:
        print(
            Fore.YELLOW
            + "YELLOW\tContent of %s files" % str(fileCount - 2)
            + Fore.WHITE
        )
    if fileCount > 3:
    ...
    if fileCount > 4:
        print(
            Fore.MAGENTA
            + "MAGENTA\tContent of %s files" % str(fileCount - 4)
            + Fore.WHITE
        )
    if fileCount > 5:
        print(
            Fore.WHITE + "WHITE\tContent of %s files" % str(fileCount - 5) + Fore.WHITE
        )
    if fileCount > 6:
        print(
            Fore.WHITE
            + Style.DIM
            + "..\tNo identical bytes in more than %s files" % str(fileCount - 6)
            + Fore.WHITE
            + Style.RESET_ALL
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-m", action="store_true", default=False, help="Heat map on byte values"
    )
    parser.add_argument(
        "-f",
        default=" ",
        metavar="byte-filler",
        help="character to fill the gap between the bytes (default: ' ')",
    )
    parser.add_argument(
        "-c",
        default=None,
        metavar="num-occurances",
        help="Print only bytes that occur in at least X of the samples (default: all files; incompatible with heat map mode) ",
    )
    parser.add_argument(
        "-b",
        default=1024,
        metavar="bytes",
        help="Number of bytes to print (default: 1024)",
    )
    parser.add_argument(
        "-l",
        default=16,
        metavar="bytes-per-line",
        help="Number of bytes to print per line (default: 16)",
    )
