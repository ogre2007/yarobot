...
lazy_static! {
    pub static ref REGEX_INSENSITIVE: RegexRules = {
        ...
        m.insert("copyright", vec![(r"(coded | c0d3d |cr3w\b|Coded by |codedby)", 7)]);
        m.insert("extensions_generic", vec![(r"\.[a-zA-Z]{3}\b", 3)]);
        m.insert("alll_characters", vec![(r"^[A-Z][a-z]{5,}$", 2)]);
        m.insert("URL", vec![(r"(%[a-z][:\-,;]|\\\\%s|\\\\[A-Z0-9a-z%]+\\[A-Z0-9a-z%]+)", 2)]);
        m.insert("certificates", vec![(r"(thawte|trustcenter|signing|class|crl|certificate|assembly)", -4)]);
        m.insert("parameters1", vec![(r"( \-[a-z]{,2}[\s]?[0-9]?| /[a-z]+[\s]?[\w]*)", 4)]);
        m.insert("directory", vec![(r"([a-zA-Z]:|^|%)\\[A-Za-z]{4,30}\\", 4)]);
        m.insert("executable_no_dir", vec![(r"^[^\\]+\.(exe|com|scr|bat|sys)$", 4)]);
        m.insert("date_placeholders", vec![(r"(yyyy|hh:mm|dd/mm|mm/dd|%s:%s:)", 3)]);
        m.insert("placeholders", vec![(r"[^A-Za-z](%s|%d|%i|%02d|%04d|%2d|%3s)[^A-Za-z]", 3)]);
        m.insert("string_parts", vec![(r"(cmd|com|pipe|tmp|temp|recycle|bin|secret|private|AppData|driver|config)", 3)]);
        m.insert(
            "programming",
            vec![(
        ...
        m.insert("special_strings", vec![(r"(\\\\\.\\|kernel|.dll|usage|\\DosDevices\\)", 5)]);
        m.insert("parameters", vec![(r"( \-[a-z] | /[a-z] | \-[a-z]:[a-zA-Z]| \/[a-z]:[a-zA-Z])", 4)]);
        m.insert("file", vec![(r"^[a-zA-Z0-9]{3,40}\.[a-zA-Z]{3}", 3)]);
        m.insert("comment", vec![(r"^([\*\#]+ |\[[\*\-\+]\] |[\-=]> |\[[A-Za-z]\] )", 4)]);
        m.insert("typo", vec![(r"(!\.$|!!!$| :\)$| ;\)$|fucked|[\w]\.\.\.\.$)", 4)]);
        m.insert("base64", vec![(r"^(?:[A-Za-z0-9+/]{4}){30,}(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$", 7)]);
        m.insert(
            "b64exec",
            vec![(
        ...
        m.insert("privilege", vec![(r"(administrator|highest|system|debug|dbg|admin|adm|root) privelege", 4)]);
        m.insert("system_file", vec![(r"(LSASS|SAM|lsass.exe|cmd.exe|LSASRV.DLL)", 4)]);
        ...
        m.insert("signing_certificates", vec![(r"( Inc | Co.|  Ltd.,| LLC| Limited)", 2)]);
        m.insert("privilege_escalation", vec![(r"(sysprep|cryptbase|secur32)", 2)]);
        m.insert("webshells", vec![(r"(isset\($post\[|isset\($get\[|eval\(Request)", 2)]);
        m.insert(
            "suspicious_words",
            vec![(
        ...
        m.insert("mutex_pipes", vec![(r"(Mutex|NamedPipe|\\Global\\|\\pipe\\)", 3)]);
        m.insert("usage", vec![(r"(isset\($post\[|isset\($get\[)", 2)]);
        m.insert("hash", vec![(r"\b([a-f0-9]{32}|[a-f0-9]{40}|[a-f0-9]{64})\b", 2)]);
        m.insert("persistence", vec![(r"(sc.exe |schtasks|at \\\\|at [0-9]{2}:[0-9]{2})", 3)]);
        m.insert(
            "unix",
            vec![(
        ...
        m.insert("less_value", vec![(r"(abcdefghijklmnopqsst|ABCDEFGHIJKLMNOPQRSTUVWXYZ|0123456789:;)", -5)]);
        m.insert("vb_backdoors", vec![(r"(kill|wscript|plugins|svr32|Select )", 3)]);
        m.insert("susp_strings_combo", vec![(r"([a-z]{4,}[!\?]|\[[!+\-]\] )", 3)]);
        m.insert("special_chars", vec![(r"(-->|!!!| <<< | >>> )", 5)]);
        m.insert("swear", vec![(r"\b(fuck|damn|shit|penis|nigger)\b", 5)]);
        m.insert("scripts", vec![(r"(%APPDATA%|%USERPROFILE%|Public|Roaming|& del|& rm| && |script)", 3)]);
    ...
    pub static ref REGEX_SENSITIVE: RegexRules = {
        ...
        m.insert("ua_keywords", vec![(r"(Mozilla|MSIE|Windows NT|Macintosh|Gecko|Opera|User\-Agent)", 5)]);
